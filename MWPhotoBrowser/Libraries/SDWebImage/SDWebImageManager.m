...
- (void)cancelForDelegate:(id<SDWebImageManagerDelegate>)delegate
{
    ...
    {
        SDWebImageDownloader *downloader = SDWIReturnRetained(downloaders[idx]);
...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if (info[@"success"])
    {
        SuccessBlock success = info[@"success"];
        success(image);
    }
#endif
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didUpdatePartialImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            {
                NSDictionary *userInfo = downloadInfo[uidx][@"userInfo"];
                if ([userInfo isKindOfClass:NSNull.class])
                {
                    userInfo = nil;
                }
...
#if NS_BLOCKS_AVAILABLE
                if (downloadInfo[uidx][@"success"])
                {
                    SuccessBlock success = downloadInfo[uidx][@"success"];
                    success(image);
                }
#endif
            }
...
#if NS_BLOCKS_AVAILABLE
                if (downloadInfo[uidx][@"failure"])
                {
                    FailureBlock failure = downloadInfo[uidx][@"failure"];
                    failure(nil);
                }
#endif
            }
